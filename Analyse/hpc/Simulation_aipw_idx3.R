...
writeLines(c("Log of iterations"), paste0(path_work, "log_aipw3.txt"))
...
result <-  foreach(i = (idx.row2+1):idx.row3, .combine = rbind, .options.future=list(seed=TRUE)) %dofuture% {
    ...
    sink(paste0(path_work, "log_aipw3.txt"), append=TRUE)
    on.exit(sink())
...
save(res, file = paste0(path_work,"results_aipw3.Rdata"))
