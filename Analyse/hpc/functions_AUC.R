...
conv <- function(data, d, ind, cov_pred, cov_prop, tt){
  col1 <- match(paste(ind),names(data)) # column number of index test
  col2 <- match(paste(d),names(data)) # column number of disease variable
  x <- data[which(data[,col2]==1), col1] # only index test values of diseased sample
  y <- data[which(data[,col2]==0), col1] # only index test values of healthy sample
  deltax <- ifelse((is.na(x)), 0, 1) # missingness indicator
  deltay <- ifelse((is.na(y)), 0, 1)
  zxprop <- data[which(data[,col2]==1), cov_prop]
  zyprop <- data[which(data[,col2]==0), cov_prop]
  zxpred <- data[which(data[,col2]==1), cov_pred]
  zypred <- data[which(data[,col2]==0), cov_pred]
  ...
  if (sum(!is.na(x))==0 | sum(!is.na(y))==0) {
    ...
    # logistic model
    covars <- paste(cov_prop, collapse = "+") # select covariates
    form <- paste("deltax ~",covars)
    a <- glm(as.formula(form), data=zxprop, family = "binomial")
    px <- unname(a$fitted.values)
    form <- paste("deltay ~", covars)
    b <- glm(as.formula(form), data=zyprop, family = "binomial")
    py <- unname(b$fitted.values)
    
    # regression model with only observed data
    xobs <- x[deltax==1]
    yobs <- y[deltay==1]
    zxobs <- zxpred[deltax==1,]
    zyobs <- zypred[deltay==1,]
    covars <- paste(cov_pred, collapse = "+") # select covariates
    form <- paste("xobs ~", covars)
    lmfitx <- lm(as.formula(form), data = zxobs)
    m.estx <- predict.lm(lmfitx, newdata=zxpred)
    form <- paste("yobs ~", covars)
    lmfity <- lm(as.formula(form), data=zyobs)
    m.esty <- predict.lm(lmfity, newdata=zypred)
    
    # calculation of ROC (sensitivity values for respective false positive rate)
    ROC.CONV=rep(NA,length(tt))
    for(i in 1:length(tt)){
      ROC.CONV[i]<-roc_CONV(y_D=x,y_H=y,deltax,deltay,pyx_D=px,pyx_H=py,m.estx,m.esty,p=tt[i])
    }
    
    # calculation of AUC
    AUC.CONV=mean(ROC.CONV)
    # calculate CI LG
    n <- length(x)
    m <- length(y)
    q0=AUC.CONV*(1-AUC.CONV)
    q1=(AUC.CONV/(2-AUC.CONV))-AUC.CONV^2
    q2=(2*AUC.CONV^2)/(1+AUC.CONV)-AUC.CONV^2
    CIU.CONV=AUC.CONV+1.96*sqrt((q0+(n-1)*q1+(m-1)*q2)/(n*m))
    CIL.CONV=AUC.CONV-1.96*sqrt((q0+(n-1)*q1+(m-1)*q2)/(n*m))
  }
  
  return(list(AUC.CONV=AUC.CONV, CIU.CONV=CIU.CONV, CIL.CONV=CIL.CONV, ROC.CONV=ROC.CONV))
}

# intern function

# Parameters:
# y_D = index test values of diseased sample
# y_H = index test values of non-diseased sample
# delta_D = missingness indicator of diseased sample
# delta_H = missingness indicator of non-diseased sample
# pyx_D = fitted values/predicted probabilites of the logistic model for the diseased sample
# pyx_H = fitted values/predicted probabilites of the logistic model for the non-diseased sample
# m.est_D = predicted values for the diseased subjects based on the linear model
# m.est_H = predicted values for the non-diseased subjects based on the linear model
# p = false positive rate

roc_CONV<- function(y_D,y_H,delta_D,delta_H,pyx_D,pyx_H, m.est_D,m.est_H, p) 
{
  
  conv <- function(y, delta, m.est){
    n=length(y)
    tau=rep(1,n)/n
    kapa=delta/sum(delta)
    pes=ysomb=matrix(rep(0,n*n),n,n)
  ...
  res <- conv(y=y_H, delta=delta_H, m.est=m.est_H)
  
  pes.vec=as.vector(res[[1]])
  ysomb.vec=as.vector(res[[2]]) # for healthy sample
  
  ysomb.vec[pes.vec==0]=NA
  
  w <- pes.vec/sum(pes.vec) 
  a <- p
  b <- 1-p
  ox <- order(ysomb.vec)
  y <- ysomb.vec[ox] # order y increasing (NAs last)
  w <- w[ox] 
  k <- 1
  low <- cumsum(c(0, w)) # cumulative sum, starting with extra value 0
  up <- sum(w) - low # inverse of low
  df <- a * low - b * up
  ...
  if (df[k] == 0) {
    punto <- (w[k] * y[k] + w[k-1] * y[k-1]) / (w[k] + w[k-1])
  } else {
    punto <- y[k-1]
  }
  lp=length(punto)
  
  res <- conv(y=y_D, delta=delta_D, m.est=m.est_D)
  pes=res[[1]]
  ysomb=res[[2]]
  
  FY.conv <- rep(NA,lp)
  for (ele in 1:lp){
    FY.conv[ele]=sum(pes*1*(ysomb<=punto[ele]), na.rm=TRUE)
  }
  roc_conv <- 1-FY.conv
  return(roc_conv)
}
...
ker <- function(data, d, ind, cov_pred, cov_prop, tt){
  ...
  if (sum(!is.na(x))==0 | sum(!is.na(y))==0) {
    ...
    # calculation of ROC (sensitivity values for respective false positive rate)
    ROC <- rep(NA, length(tt))
    for (i in 1:length(tt)){
      ROC[i] <- roc_KER(x,y,deltax,deltay,px,py,p=tt[i])
    }
    # calculation of AUC
    AUC.KER=mean(ROC)
    # calculate CI LG
    n <- length(x)
    m <- length(y)
    q0=AUC.KER*(1-AUC.KER)
    q1=(AUC.KER/(2-AUC.KER))-AUC.KER^2
    q2=(2*AUC.KER^2)/(1+AUC.KER)-AUC.KER^2
    CIU.KER=AUC.KER+1.96*sqrt((q0+(n-1)*q1+(m-1)*q2)/(n*m))
    CIL.KER=AUC.KER-1.96*sqrt((q0+(n-1)*q1+(m-1)*q2)/(n*m))
  }
  
  return(list(AUC.KER=AUC.KER, CIU.KER=CIU.KER, CIL.KER=CIL.KER, ROC.KER=ROC))
}
...
roc_KER <- function(y_D,y_H,delta_D,delta_H,pyx_D,pyx_H, p){
  ...
  cn=1+1.8*n_D^(-1/5)
  ache= cn*sqrt(5*p*(1-p))/sqrt(2*n_H) # bandwidth
  ...
  for (i in 1:n_H){
    ...
    tau1[i]=  delta_H[i]/pyx_H[i]   # IPW for diseased sample
    
  }
  
  tau= tau1/sum(tau1) # scale to 0,1
  
  Z=rep(NA,n_D)
  
  for (i in 1:n_D){
    punto=y_D[i] # here punto = normal diseased y data
    FY.IPW=sum(tau*1*(y_H<=punto),na.rm=TRUE)
    Z[i]=1-FY.IPW 
  }
  ...
  for (i in 1:nsamp){
    ...
    tau1[i]=  delta_D[i]/pyx_D[i]    # for non-diseased sample
    
  }
  tau= tau1/sum(tau1)
  
  arg=(p-Z)/ache # kernel function(p-Z)/h
  roc_KER =sum(tau*nucleo(arg),na.rm=TRUE)
