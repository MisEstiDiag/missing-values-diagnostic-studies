...
writeLines(c("Log of iterations"), paste0(path_work, "log_mice3.txt"))
...
suppressMessages({
  ...
  set.seed(952)
  result <- foreach(i = (idx.row2+1):idx.row3, .combine = rbind, .options.future=list(seed=TRUE)) %dofuture% {
      
      sink(paste0(path_work, "log_mice3.txt"), append=TRUE)
      on.exit(sink())
...
res <- cbind(grid[(idx.row2+1):idx.row3,],result)
tfull2 <- proc.time()
time_sim <- tfull2-tfull1
time_sim

save(res, file = paste0(path_work,"results_micemix2_3.Rdata"))
