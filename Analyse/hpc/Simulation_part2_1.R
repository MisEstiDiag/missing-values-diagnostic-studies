...
writeLines(c("Log of iterations"), paste0(path_work, "log_part2_1.txt"))
...
suppressMessages({
  ...
  result <- foreach(i = 1:idx.row1, .combine = rbind, .options.future=list(seed=TRUE)) %dofuture% {
      ...
      sink(paste0(path_work, "log_part2_1.txt"), append=TRUE)
      on.exit(sink())
...
save(res, file = paste0(path_work,"results_part2.Rdata"))
