...
writeLines(c("Log of iterations"), paste0(path_work, "log_mice1.txt"))
...
suppressMessages({
  ...
  result <- foreach(i = 1:idx.row1, .combine = rbind, .options.future=list(seed=TRUE)) %dofuture% {
      ...
      sink(paste0(path_work, "log_mice1.txt"), append=TRUE)
      on.exit(sink())
...
save(res, file = paste0(path_work,"results_micemix2_1.Rdata"))
