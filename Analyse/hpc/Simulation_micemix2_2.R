...
writeLines(c("Log of iterations"), paste0(path_work, "log_mice2.txt"))
...
suppressMessages({
  ...
  result <- foreach(i = (idx.row1+1):idx.row2, .combine = rbind, .options.future=list(seed=TRUE)) %dofuture% {
      ...
      sink(paste0(path_work, "log_mice2.txt"), append=TRUE)
      on.exit(sink())
...
save(res, file = paste0(path_work,"results_micemix2_2.Rdata"))
