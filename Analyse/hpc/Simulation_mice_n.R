...
writeLines(c("Log of iterations"), paste0(path_work, "log_mice_n.txt"))
...
suppressMessages({
  ...
  result <- foreach(i = 1:nrow(grid), .combine = rbind, .options.future=list(seed=TRUE)) %dofuture% {
      ...
      sink(paste0(path_work, "log_mice_n.txt"), append=TRUE)
      on.exit(sink())
      ...
      var.between = (sum((fit-AUC.mice_n)^2))/(length(fit)-1)
      se.mice = sqrt(var.within+var.between+(var.between/length(se))) # pooled SE
      CIU.mice_n = AUC.mice_n+1.96*se.mice # confidence interval
      CIL.mice_n = AUC.mice_n-1.96*se.mice
      t2 <- proc.time()
      time_sim <- t2-t1
      time.mice_n <- time_sim[3]
      
      res <- c(AUC.mice_n, CIL.mice_n, CIU.mice_n, time.mice_n)
  
    }
})

colnames(result) <- c("AUC.mice_n","CIL.mice_n","CIU.mice_n","time.mice_n")
...
save(res, file = paste0(path_work,"results_mice_n.Rdata"))
