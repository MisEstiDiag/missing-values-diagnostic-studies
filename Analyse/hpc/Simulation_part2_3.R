...
suppressMessages({
  ...
  set.seed(952)
  result <- foreach(i = (idx.row2+1):nrow(grid), .combine = rbind, .options.future=list(seed=TRUE)) %dofuture% {
...
res <- cbind(grid[(idx.row2+1):nrow(grid),],result)
tfull2 <- proc.time()
time_sim <- tfull2-tfull1
time_sim

save(res, file = paste0(path_work,"results_part2_3.Rdata"))
