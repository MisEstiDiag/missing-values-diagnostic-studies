...
writeLines(c("Log of iterations"), paste0(path_work, "log_ipl.txt"))
...
suppressMessages({
  ...
  result <- foreach(i = 1:nrow(grid), .combine = rbind, .options.future=list(seed=TRUE)) %dofuture% {
    ...
    # estimate AUC + CI
      ...
      # IPL LG & NP
      t1 <- proc.time()
      res <- ipl(data=data, d="D", ind="V1", cov=c("V2", "V3", "V4"), tt=seq(0.01, 1-0.01, 0.01), kLevel=0.95, kK=20, seed=184*i)
      AUC.IPL.LG <- (res$res)$auc[1]
      AUC.IPL.NP <- (res$res)$auc[2]
      CIU.IPL.LG <- (res$res)$aucupp[1]
      CIL.IPL.LG <- (res$res)$auclow[1]
      CIU.IPL.NP <- (res$res)$aucupp[2]
      CIL.IPL.NP <- (res$res)$auclow[2]
      t2 <- proc.time()
      time_sim <- t2-t1
      time.IPL <- time_sim[3]
      
  
    res <-  c(AUC.IPL.LG,CIL.IPL.LG,CIU.IPL.LG,AUC.IPL.NP,CIL.IPL.NP,CIU.IPL.NP,time.IPL)
    
  }

  colnames(result) <- c("AUC.IPL.LG","CIL.IPL.LG","CIU.IPL.LG","AUC.IPL.NP","CIL.IPL.NP","CIU.IPL.NP","time.IPL")
})
...
save(res, file = paste0(path_work,"results_ipl.Rdata"))
