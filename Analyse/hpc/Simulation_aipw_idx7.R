...
idx.row2 <- idx.row1*2
idx.row3 <- idx.row1*3 # iteration idx.row3 - idx.row1*4 takes too much time --> divide further
idx.row4 <- (idx.row1*3)+(idx.row1/2)
idx.row5 <- idx.row1*4
idx.row6 <- (idx.row1*4)+(idx.row1/2)
#idx-row7 <- nrow(grid)
...
writeLines(c("Log of iterations"), paste0(path_work, "log_aipw7.txt"))
...
tfull1 <- proc.time()
set.seed(952)
result <-  foreach(i = (idx.row6+1):nrow(grid), .combine = rbind, .options.future=list(seed=TRUE)) %dofuture% {
    
    
    sink(paste0(path_work, "log_aipw7.txt"), append=TRUE)
    on.exit(sink())
...
res <- cbind(grid[(idx.row6+1):nrow(grid),],result)

tfull2 <- proc.time()
time_sim <- tfull2-tfull1
time_sim

save(res, file = paste0(path_work,"results_aipw7.Rdata"))
