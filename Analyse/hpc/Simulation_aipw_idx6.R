...
writeLines(c("Log of iterations"), paste0(path_work, "log_aipw6.txt"))
...
tfull1 <- proc.time()
set.seed(952)
result <-  foreach(i = (idx.row5+1):idx.row6, .combine = rbind, .options.future=list(seed=TRUE)) %dofuture% {
    
    
    sink(paste0(path_work, "log_aipw6.txt"), append=TRUE)
    on.exit(sink())
...
res <- cbind(grid[(idx.row5+1):idx.row6,],result)

tfull2 <- proc.time()
time_sim <- tfull2-tfull1
time_sim

save(res, file = paste0(path_work,"results_aipw6.Rdata"))
