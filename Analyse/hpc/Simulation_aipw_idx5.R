...
writeLines(c("Log of iterations"), paste0(path_work, "log_aipw5.txt"))
...
tfull1 <- proc.time()
set.seed(952)
result <-  foreach(i = (idx.row4+1):idx.row5, .combine = rbind, .options.future=list(seed=TRUE)) %dofuture% {
    
    
    sink(paste0(path_work, "log_aipw5.txt"), append=TRUE)
    on.exit(sink())
...
res <- cbind(grid[(idx.row4+1):idx.row5,],result)

tfull2 <- proc.time()
time_sim <- tfull2-tfull1
time_sim

save(res, file = paste0(path_work,"results_aipw5.Rdata"))
