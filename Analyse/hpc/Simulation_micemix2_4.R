...
idx.row2 <- idx.row1*2
idx.row3 <- (idx.row1*2)+(idx.row1/2)

######  Start Simulation  ######

# document progress: log file
writeLines(c("Log of iterations"), paste0(path_work, "log_mice4.txt"))
...
suppressMessages({
  ...
  set.seed(952)
  result <- foreach(i = (idx.row3+1):nrow(grid), .combine = rbind, .options.future=list(seed=TRUE)) %dofuture% {
      
      sink(paste0(path_work, "log_mice4.txt"), append=TRUE)
      on.exit(sink())
      ...
      rngseed(15*i) # set random number generator seed, must be set (otherwise da.mix wont work), change for simulation
      implist=list()
      ...
      time.mix <- time_sim[3]
        ...
        res <- c(AUC.mice,CIL.mice,CIU.mice,time.mice,AUC.mix,CIL.mix,CIU.mix,time.mix)
  
    }
})

colnames(result) <- c("AUC.mice","CIL.mice","CIU.mice","time.mice","AUC.mix","CIL.mix","CIU.mix","time.mix")


res <- cbind(grid[(idx.row3+1):nrow(grid),],result)
tfull2 <- proc.time()
time_sim <- tfull2-tfull1
time_sim

save(res, file = paste0(path_work,"results_micemix2_4.Rdata"))
