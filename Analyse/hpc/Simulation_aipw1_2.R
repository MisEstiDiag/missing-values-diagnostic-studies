...
writeLines(c("Log of iterations"), paste0(path_work, "aipw1_2.txt"))
...
suppressMessages({
  ...
  tfull1 <- proc.time()
  set.seed(5273)
  result <- foreach(i = (idx.row1+1):nrow(grid), .combine = rbind, .options.future=list(seed=TRUE)) %dofuture% {
      
      sink(paste0(path_work, "aipw1_2.txt"), append=TRUE)
      on.exit(sink())
...
res <- cbind(grid[(idx.row1+1):nrow(grid),],result)
tfull2 <- proc.time()
time_sim <- tfull2-tfull1
time_sim

save(res, file = paste0(path_work,"part1_aipw2.Rdata"))
