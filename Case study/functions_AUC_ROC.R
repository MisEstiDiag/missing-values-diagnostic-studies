...
mi2 <- function(data, d, ind, cov_pred, cov_prop, w=c(0.5,0.5), kN=3, kL=10, tt, kLevel=0.95, seed) {
  ...
  if (nrow(xx[which(xx$delta==1), ])==0 | nrow(yy[which(yy$delta==1), ])==0) {
    ...
    ## ROC estimation
    s <- length(tt)
    roc <- matrix(0, kL, s) # matrix of ROC results (sens for given 1-spec rate)
    var.roc <- matrix(0, kL, s) 
    for (k in 1:kL){ 
      x.new <- x.mi.pred[, k] # for each mi dataset separate calculations
      y.new <- y.mi.pred[, k]
      for (i in 1:s){
        quan <- quantile(y.new, 1 - tt[i]) # depending on tt; select cut off value for the 1-tt quanitle (= sensitivity, e.g. 80%)
        roc[k, i] <- mean(1 * (x.new >= quan)) # calculate sensitivity at given cut off value
        den.x <- ks::kde(x.new, h=hpi(x.new), eval.point=quan)$estimate # kernel density estimate
        den.y <- ks::kde(y.new, h=hpi(y.new), eval.point=quan)$estimate
        ratio <- den.x / den.y
        var.roc[k, i] <- roc[k, i] * (1 - roc[k, i]) / m +  # variance
          (ratio)^2 * tt[i] * (1 - tt[i]) / n
      }
    }
    roc.mean <- colMeans(roc) # compute summary roc (sensitivity) across kl imputed datasets
    var.total <- colMeans(var.roc) + (1 + kL) / kL * apply(roc, 2, var) # compute total var across imputed datasets
    sd.roc.mib2 <- sqrt(var.total)
    ROC.CIL.MI2 <- roc.mean - sd.roc.mib2 * qnorm((1 + kLevel) / 2)
    ROC.CIU.MI2 <- roc.mean + sd.roc.mib2 * qnorm((1 + kLevel) / 2)    
  
  }
  
  return(list(AUC.MI2=AUC.MI2, CIU.MI2=CIU.MI2, CIL.MI2=CIL.MI2, 
              ROC.MI2=roc.mean, ROC.CIU.MI2=ROC.CIU.MI2, ROC.CIL.MI2=ROC.CIL.MI2))
}
...
PredPropScore <- function(xx, kN=kN, MI=MI, cov_pred=cov_pred, cov_prop=cov_prop, col1=col1, w=c(0.5,0.5)){
  # prediction score
...
mi2boot <- function(data, d, ind, cov_pred, cov_prop, w=c(0.5,0.5), kN=3, kL=10, tt, kLevel, seed) {
  ...
  if (nrow(xx[which(xx$delta==1), ])==0 | nrow(yy[which(yy$delta==1), ])==0) {
    ...
    for (b in 1:kL){ 
      ...
      x.mi.pred <- PredPropScore(xx=xx.boot, kN=kN, MI=1, cov_pred=cov_pred, cov_prop=cov_prop, col1=col1)
      ...
      y.mi.pred <- PredPropScore(xx=yy.boot, kN=kN, MI=1, cov_pred=cov_pred, cov_prop=cov_prop, col1=col1)
      ...
      # ROC estimation
      for (i in 1:s){
        quan <- quantile(y.mi.pred, 1 - tt[i])
        roc[b,i] <- mean(1 * (x.mi.pred >= quan))
        den.x <- ks::kde(x.mi.pred, h=hpi(x.mi.pred), eval.point=quan)$estimate
        den.y <- ks::kde(y.mi.pred, h=hpi(y.mi.pred), eval.point=quan)$estimate
        ratio <- den.x / den.y
        var.roc[b, i] <- roc[b, i] * (1 - roc[b,i]) / m + 
          ratio^2 * tt[i] * (1 - tt[i]) / n
      }
    }
    # Pool ROC
    ROC.MIB2 <- colMeans(roc)
    var.total <- colMeans(var.roc) + (1 + kL) / kL * apply(roc, 2, var) 
    sd.roc.mib2 <- sqrt(var.total)
    ROC.CIL.MIB2 <- ROC.MIB2 - sd.roc.mib2 * qnorm((1 + kLevel) / 2)
    ROC.CIU.MIB2 <- ROC.MIB2 + sd.roc.mib2 * qnorm((1 + kLevel) / 2) 
  ...
  return(list(AUC.MIB2=AUC.MIB2, CIU.MIB2=CIU.MIB2, CIL.MIB2=CIL.MIB2, 
              ROC.MIB2=ROC.MIB2, ROC.CIU.MIB2=ROC.CIU.MIB2, ROC.CIL.MIB2=ROC.CIL.MIB2))
}
...
ipl <- function(data, d, ind, cov, tt, kLevel=0.95, kK=20, seed){
  ...
  ii <- which.min(AIC)  # select coviariate with lowest AIC, + 1 to assign the right column in dataset
